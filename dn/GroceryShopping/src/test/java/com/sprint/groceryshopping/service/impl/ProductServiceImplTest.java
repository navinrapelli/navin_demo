...
public class ProductServiceImplTest {
	...
	public void addProductEntity() {
		Product product = new Product("Sugar", "101", "Sweet","Diablis", null);
		product.setCode(108);
	...
	public void AddProductAlreadyExists() {	
		Product product = new Product("Sugar", "101", "Sweet","Diablis", null);
		product.setCode(101);
		Mockito.when(productDao.save(product)).thenThrow(new DuplicateRecordFoundException("Product Already Exists"));
		ProductDTO dto = ProductServiceImpl.convertEntityToDTO(product);
		Exception exception = assertThrows(DuplicateRecordFoundException.class, () -> productDao.save(product));
	...
	public void UpdateNameNotPresent() {	
		Product product = new Product("Sugar", "101", "Sweet","Diablis", null);
		String name = "book";
		Mockito.when(productDao.save(product)).thenThrow(new RecordNotFoundException("Record Not Exists"));
		Exception exception = assertThrows(RecordNotFoundException.class, () -> productDao.save(product));
	...
	{
       Product product = new Product("es", "ft", "gy", "gjy", null);
		Mockito.when(productDao.findAll()).thenReturn(Stream.of(product).collect(Collectors.toList()));
		List<ProductDTO> actual = productService.getAllProducts();
		assertEquals(1,actual.size());
	}
